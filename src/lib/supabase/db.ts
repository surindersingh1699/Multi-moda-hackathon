...
export async function createShare(
...
): Promise<{ shareId: string; imageUrl: string } | null> {
  // Upload composite image to public bucket
  const bytes = Uint8Array.from(
    atob(params.compositeImageBase64.replace(/^data:image\/\w+;base64,/, "")),
    (c) => c.charCodeAt(0),
  );
  const shareId = crypto.randomUUID().slice(0, 16);
  const path = `${shareId}.jpg`;
  ...
  const { data: urlData } = supabase.storage.from("shared-images").getPublicUrl(path);
  ...
  return { shareId: share.id, imageUrl: urlData.publicUrl };
}
